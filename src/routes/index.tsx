  ...
  head: () => ({
    meta: [
      { title: "Adityahomefurniture — Handcrafted Indian Oak & Teak" },
      {
        name: "description",
        content:
          "Timeless Indian oak and teak furnishings, hand-built by master artisans. Shop sofas, dining, storage and lounge pieces.",
      },
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={hero}
            alt="Indian oak and teak living room"
        ...
        <div className="relative mx-auto flex min-h-[88vh] max-w-7xl flex-col justify-end px-4 pb-16 pt-32 sm:px-6 lg:px-8 lg:pb-24">
          ...
          <h1 className="mt-6 max-w-3xl font-serif text-4xl text-primary-foreground text-balance sm:text-6xl lg:text-7xl">
            Heirloom furniture, carved from{" "}
            <em className="italic text-teak-300">Indian oak & teak</em>.
          </h1>
          <p className="mt-5 max-w-xl text-base text-primary-foreground/85 sm:text-lg">
            Sustainably sourced timber. Hand-cut joinery. Pieces that age like a memory — warmer
            with every season.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              to="/products"
              className="inline-flex items-center gap-2 rounded-md bg-amber px-6 py-3 text-sm font-semibold text-accent-foreground shadow-elevated transition hover:opacity-90"
            >
              Explore the collection <ArrowRight className="h-4 w-4" />
            </Link>
            <Link
              to="/about"
              className="inline-flex items-center gap-2 rounded-md border border-primary-foreground/40 bg-primary-foreground/5 px-6 py-3 text-sm font-medium text-primary-foreground hover:bg-primary-foreground/15"
            >
              Our craft
            </Link>
          </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:px-8">
          {[
            ...
            <div key={t} className="flex items-start gap-3">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-background text-oak-700">
      ...
      <section id="featured" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link
            to="/products"
            className="hidden sm:inline-flex items-center gap-1 text-sm text-foreground hover:text-oak-700"
          >
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <Link to="/products" className="text-sm text-oak-700 hover:text-oak-900">
                    Details →
                  </Link>
                </div>
              </div>
      ...
      <section className="bg-oak-100">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-28">
          ...
          <div className="flex flex-col justify-center">
            ...
            <h2 className="mt-2 font-serif text-3xl sm:text-4xl text-oak-900">
              Forty hands. One bench. One season.
            </h2>
            <p className="mt-4 text-base text-oak-900/80">
              Every piece begins as a slab of seasoned Indian hardwood, hand-selected at the mill
              for grain and weight. Our master joiners spend up to twelve weeks shaping, fitting and
              oil-finishing each design — no veneers, no shortcuts.
            </p>
            <ul className="mt-6 space-y-3 text-sm text-oak-900/85">
              <li>
                <strong className="text-oak-900">Air-dried timber</strong> — 18 to 24 months before
                the workshop.
              </li>
              <li>
                <strong className="text-oak-900">Mortise & tenon</strong> — traditional joinery,
                never glue alone.
              </li>
              <li>
                <strong className="text-oak-900">Plant-based oil</strong> — finishes that breathe
                and patina.
              </li>
            </ul>
            <Link
              to="/about"
              className="mt-8 inline-flex w-fit items-center gap-2 rounded-md bg-oak-900 px-5 py-3 text-sm font-medium text-primary-foreground hover:bg-oak-700"
            >
          ...
          {testimonials.slice(0, 3).map((t) => (
            <figure key={t.id} className="rounded-xl border border-border bg-card p-6 shadow-soft">
              ...
              <figcaption className="mt-5 flex items-center gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-full bg-gradient-wood text-sm font-medium text-primary-foreground">
                  {t.initials}
                </span>
                <div>
          ...
          <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr] lg:items-center">
            <div>
              <h2 className="font-serif text-3xl sm:text-4xl">
                Designing a home, an office, a campus?
              </h2>
              <p className="mt-3 max-w-xl text-primary-foreground/80">
                We've furnished homes from Hyderabad to Bangalore and institutions including NIT
                Warangal. Tell us your space.
              </p>
            </div>
            <div className="flex flex-wrap gap-3 lg:justify-end">
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 rounded-md bg-amber px-6 py-3 text-sm font-semibold text-accent-foreground hover:opacity-90"
              >
                Start a project
              </Link>
              <Link
                to="/projects"
                className="inline-flex items-center gap-2 rounded-md border border-primary-foreground/40 px-6 py-3 text-sm font-medium hover:bg-primary-foreground/10"
              >
                See case studies
              </Link>
            </div>
